...
log_type = 'SalesforceServiceCloudLogin'
...
def pull_log_files():
    ...
    queryAudit =  "/services/data/v44.0/query?q=SELECT+ApiType+,+Application+,+Browser+,+Id+,+LoginTime+,+LoginType+,+LoginUrl+,+Platform+,+SourceIp+,+Status+,+UserId" + \
                "+FROM+LoginHistory"   + \
                f"+WHERE+LoginTime+>+{past_time}"                    
    try:
        logging.info('Searching files last modified from {}'.format(past_time))
        #r = requests.get(f'{instance_url}{query}', headers=headers)
        r = requests.get(f'{instance_url}{queryAudit}', headers=headers)
    except Exception as err:
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    for line in pull_log_files():
        ...
        if line['UserId']:
            user_email = users.get(line['UserId'])
            line['UserId'] = user_email
        del line['attributes']
